...
public abstract class KeyedResultAdapter extends QueryResultAdapter<LibraryBook> implements SectionIndexer {
	...
	public String getKey(int position) {
		...
		if (keys == null || position >= keys.size()) return null;
	...
	public int getPositionForSection(int section) {
		if (section < 0 || this.keyedResult == null) return -1;
		
		List<Character> alphabet = this.keyedResult.getAlphabet();
		
		if (section >= alphabet.size()) return 0;
	...
	public int getSectionForPosition(int position) {
		if (this.keyedResult == null) return 0;
		
		if (position < 0 || position >= this.keyedResult.getSize()) return 0;
	...
	public Object[] getSections() {
		if (this.keyedResult == null) return new Object[0];
